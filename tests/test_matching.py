...
def test_criterion_evidence_is_traceable() -> None:
    result = calculate_match(job(), candidate())
    for dimension in result["dimensions"]:
        for criterion in dimension["criteria"]:
            assert bool(criterion["evidence_sources"]) == (criterion["attainment"] > 0)
            for evidence in criterion["evidence_sources"]:
                assert evidence in dimension["evidence_sources"]
