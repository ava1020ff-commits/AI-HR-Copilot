...
st.set_page_config(page_title="AI 面试助手", page_icon="🎙️", layout="wide")
apply_saas_theme("面试助手")
render_page_header("AI 面试助手", "基于岗位要求、候选人经历与匹配结果生成结构化面试方案")
render_ai_intro("AI Interview", "生成可追溯的针对性问题，不生成录用结论。")
try:
...
if not jobs or not candidates:
    description = "需要先完成：创建岗位、导入候选人，再进行人岗匹配。"
    path = "pages/01_岗位管理.py" if not jobs else "pages/02_候选人.py"
    label = "＋ 创建岗位" if not jobs else "＋ 导入候选人"
    render_empty_state("◉", "还不能生成面试方案", description, action_path=path, action_label=label)
    st.stop()
selectors = st.columns(2)
job_id = selectors[0].selectbox("岗位", [item["id"] for item in jobs], format_func=lambda value: next(f"{item['label']} · {mode_label(item['mode'])}" for item in jobs if item["id"] == value))
candidate_id = selectors[1].selectbox("候选人", [item["id"] for item in candidates], format_func=lambda value: next(item["label"] for item in candidates if item["id"] == value))
...
if job["mode"] == "mock":
    st.warning("此岗位为示例模式数据，生成的面试提纲仅供体验。")
if use_current:
...
if "interview_result" in st.session_state:
    ...
    guide = saved["guide"]
    render_section_title("AI 已生成结构化面试方案", "请 HR 根据面试时长选用并核对引用", ai=True)
