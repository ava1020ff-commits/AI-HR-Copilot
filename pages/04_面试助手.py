...
st.info("根据已保存资料生成面试问题，不生成录用结论，不要求提供前雇主机密或个人敏感信息。")
try:
...
if not jobs or not candidates:
    ...
    st.stop()
job_id = st.selectbox("选择岗位", [item["id"] for item in jobs], format_func=lambda value: next(f"#{item['id']} · {item['label']} · {mode_label(item['mode'])}" for item in jobs if item["id"] == value))
