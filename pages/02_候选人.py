...
library_panel = st.container(key="candidate_library")
with st.expander("导入候选人 / 解析与复核", expanded="resume_draft" in st.session_state):
    ...
    if "resume_draft" in st.session_state:
        ...
        if "resume_saved" in st.session_state:
            ...
            with st.expander("查看已保存的结构化记录"):
                st.json(saved["result"])
        else:
            with st.expander("查看初始解析详情"):
